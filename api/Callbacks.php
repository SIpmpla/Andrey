...
class Callbacks extends Simpla
{
	...
	{
		$query = $this->db->placehold("SELECT c.id, c.name, c.phone, c.message, c.date, c.processed FROM __callbacks c WHERE id=? LIMIT 1", intval($id));
	...
	{	
		...
		$query = $this->db->placehold("SELECT c.id, c.name, c.phone, c.date, c.message, c.processed
